...
static struct {     // LINEAR
    ...
    enum {LINEAR_RES_1 = 0, LINEAR_RES_2, LINEAR_RES_3, LINEAR_RES_4, LINEAR_RES_5, LINEAR_RES_6, LINEAR_RES_7, LINEAR_RES_8, LINEAR_RES_9, LINEAR_RES_END}mode;
    ...
    {LINEAR_RES_8, {3864, 2192, 3, 30}, {12, 16, 3840, 2160}, {"3840x2160@30fps"}}, // Modify it
...
static int pCus_init_8m_60fps_mipi4lane_linear(ms_cus_sensor *handle)
{
    //imx415_params *params = (imx415_params *)handle->private_data;
    int i,cnt=0;
    //s16 sen_data;
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_8m_60fps_init_table_4lane_linear);i++)
    {
        if(Sensor_8m_60fps_init_table_4lane_linear[i].reg==0xffff)
        {
            SENSOR_MSLEEP(Sensor_8m_60fps_init_table_4lane_linear[i].data);
        }
        else
        {
            cnt = 0;
            while(SensorReg_Write(Sensor_8m_60fps_init_table_4lane_linear[i].reg, Sensor_8m_60fps_init_table_4lane_linear[i].data) != SUCCESS)
            {
                cnt++;
                if(cnt>=10)
                {
...
static int pCus_SetVideoRes(ms_cus_sensor *handle, u32 res_idx)
{
	...
	SENSOR_EMSG("[%s]  Sensor res_idx: [%d] in pCus_SetVideoRes !!!!\n", __FUNCTION__,res_idx);
    ...
    switch (res_idx) {
        ...
        case 6:
           ...
           handle->pCus_sensor_init = pCus_init_8m_60fps_mipi4lane_linear;
           vts_30fps = 2250;
           params->expo.vts = vts_30fps;
           params->expo.fps = 60; //max 60,default 30
...
static int pCus_SetVideoRes_HDR_DOL_LEF(ms_cus_sensor *handle, u32 res_idx)
{
	...
	SENSOR_EMSG("[%s]  Sensor res_idx: [%d]  pCus_SetVideoRes_HDR_DOL_LEF !!!!\n", __FUNCTION__,res_idx);
...
static int pCus_SetVideoRes_HDR_DOL_SEF(ms_cus_sensor *handle, u32 res_idx)
{
	...
	SENSOR_EMSG("[%s]  Sensor res_idx: [%d]  pCus_SetVideoRes_HDR_DOL_SEF !!!!\n", __FUNCTION__,res_idx);
    handle->video_res_supported.ulcur_res = res_idx;
    ...
    switch (res_idx) {
        ...
        case HDR_RES_2:
            ...
            params->expo.vts = vts_30fps_HDR_DOL_4lane;
            params->expo.fps = 20;
            Preview_line_period_HDR_DOL_4LANE = 7246;
            handle->data_prec = CUS_DATAPRECISION_10;
            params->min_rhs1 = 437;     // 4n+1 fix to 269, 337 //5ms: 429
            break;
        case HDR_RES_3:
        ...
        case HDR_RES_7:
            ...
            params->expo.vts = vts_30fps_HDR_DOL_4lane;
            params->expo.fps = 30;
            Preview_line_period_HDR_DOL_4LANE = 7259;
            handle->data_prec = CUS_DATAPRECISION_12;
            params->min_rhs1 = 437;     // 4n+1 fix to 269, 337 //5ms: 429
            break;
        default:
