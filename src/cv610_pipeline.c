...
#define ot_err_id(ret)  ((ret) & 0x1fff)

#define chk_or_busy(expr) do { \
        td_s32 _r = (expr); \
        if (_r != TD_SUCCESS && ot_err_id(_r) != OT_ERR_BUSY) { \
            fprintf(stderr, "FAIL %s = 0x%x\n", #expr, _r); \
            return -1; \
        } \
        printf("  ok  %s%s\n", #expr, (_r == TD_SUCCESS) ? "" : "  (already up)"); \
    } while (0)
...
static int mipi_setup(const cfg_t *c)
{
...
static int sys_setup(const cfg_t *c)
{
    ot_vb_cfg vb;
    ...
    chk_or_busy(ss_mpi_vb_set_cfg(&vb));
    chk_or_busy(ss_mpi_vb_init());
    if (c->vi_online) {
        ...
        chk(ss_mpi_sys_init());
        chk(ss_mpi_sys_set_vi_vpss_mode(&vi_vpss_mode));
        chk(ss_mpi_sys_set_vi_aiisp_mode(VI_PIPE, OT_VI_AIISP_MODE_DEFAULT));
        memset(&vi_vpss_mode, 0, sizeof(vi_vpss_mode));
        chk(ss_mpi_sys_get_vi_vpss_mode(&vi_vpss_mode));
...
static int vi_setup(const cfg_t *c)
{
    ...
    chk(ss_mpi_vi_set_dev_attr(VI_DEV, &dev_attr));
    chk(ss_mpi_vi_enable_dev(VI_DEV));
    chk(ss_mpi_vi_bind(VI_DEV, VI_PIPE));
    ...
    chk(ss_mpi_vi_create_pipe(VI_PIPE, &pipe_attr));
    /* The ISP's mem_init queries pipe size from VI, so the pipe must exist. */
    chk(ss_mpi_vi_start_pipe(VI_PIPE));
    return 0;
}

static int vi_start_chn(const cfg_t *c)
{
    ...
    chk(ss_mpi_vi_set_chn_attr(VI_PIPE, VI_CHN, &chn_attr));
    chk(ss_mpi_vi_enable_chn(VI_PIPE, VI_CHN));
...
static int sensor_setup(int i2c_bus)
{
    ...
    if (g_sns_obj->pfn_set_bus_info != NULL) {
        ...
        chk(g_sns_obj->pfn_set_bus_info(VI_PIPE, bus));
    }

    chk(ss_mpi_ae_register(VI_PIPE, &g_ae_lib));
    chk(ss_mpi_awb_register(VI_PIPE, &g_awb_lib));
    chk(g_sns_obj->pfn_register_callback(VI_PIPE, &g_ae_lib, &g_awb_lib));
    return 0;
}

static int isp_setup(const cfg_t *c)
{
    ot_isp_pub_attr pub;
    ...
    chk(ss_mpi_isp_set_bind_attr(VI_PIPE, &bind));

    chk(ss_mpi_isp_mem_init(VI_PIPE));
    chk(ss_mpi_isp_set_pub_attr(VI_PIPE, &pub));
    chk(ss_mpi_isp_init(VI_PIPE));
    return 0;
}
