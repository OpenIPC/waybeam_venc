	...
	Star6eOutput *output, const RtpSidecarEncInfo *enc_info)
{
	...
	if (!state || !output || state->sidecar.fd < 0)
		return;

	rtp_sidecar_poll(&state->sidecar);

	frame_rtp_ts = state->rtp_state.timestamp;
	seq_at = state->rtp_state.seq;
	...
	if (output->ring) {
		...
		if (venc_ring_get_fill(output->ring, &fill) == 0) {
			...
			tinfo_ptr = &tinfo;
	           ...
	           output->socket_handle >= 0) {
		if (output_socket_get_fill_pct(output->socket_handle,
		    output->send_buf_capacity, &tinfo.fill_pct) == 0)
			tinfo_ptr = &tinfo;
	}

	rtp_sidecar_send_frame_transport(&state->sidecar,
		state->rtp_state.ssrc, frame_rtp_ts,
		seq_at, 0 /* seq_count: no packets sent on skip */,
		0 /* capture_us unavailable on skip path */, now_us,
		enc_info, tinfo_ptr);
}
