...
int rk_video_set_bitrate(char index, unsigned int bitrate, unsigned int max_bitrate)
{
    if (!bitrate)
        return -EINVAL;
    ...
    rk_venc_chn channel;
    memset(&channel, 0, sizeof(channel));
    int ret = rk_venc.fnGetChannelConfig(index, &channel);
    if (ret)
        return ret;

    unsigned int bounded_max = max_bitrate < bitrate ? bitrate : max_bitrate;

    switch (channel.attrib.codec) {
        case RK_VENC_CODEC_H264:
            switch (channel.rate.mode) {
                case RK_VENC_RATEMODE_H264CBR:
                    channel.rate.h264Cbr.bitrate = bitrate;
                    break;
                case RK_VENC_RATEMODE_H264VBR:
                    channel.rate.h264Vbr.bitrate = bitrate;
                    channel.rate.h264Vbr.maxBitrate = bounded_max;
                    break;
                case RK_VENC_RATEMODE_H264AVBR:
                    channel.rate.h264Avbr.bitrate = bitrate;
                    channel.rate.h264Avbr.maxBitrate = bounded_max;
                    break;
                default:
                    return -ENOTSUP;
            }
            break;
        case RK_VENC_CODEC_H265:
            switch (channel.rate.mode) {
                case RK_VENC_RATEMODE_H265CBR:
                    channel.rate.h265Cbr.bitrate = bitrate;
                    break;
                case RK_VENC_RATEMODE_H265VBR:
                    channel.rate.h265Vbr.bitrate = bitrate;
                    channel.rate.h265Vbr.maxBitrate = bounded_max;
                    break;
                case RK_VENC_RATEMODE_H265AVBR:
                    channel.rate.h265Avbr.bitrate = bitrate;
                    channel.rate.h265Avbr.maxBitrate = bounded_max;
                    break;
                default:
                    return -ENOTSUP;
            }
            break;
        case RK_VENC_CODEC_MJPG:
            switch (channel.rate.mode) {
                case RK_VENC_RATEMODE_MJPGCBR:
                    channel.rate.mjpgCbr.bitrate = bitrate;
                    break;
                case RK_VENC_RATEMODE_MJPGVBR:
                    channel.rate.mjpgVbr.bitrate = bitrate;
                    channel.rate.mjpgVbr.maxBitrate = bounded_max;
                    break;
                default:
                    return -ENOTSUP;
            }
    ...
    return rk_venc.fnSetChannelConfig(index, &channel);
}
