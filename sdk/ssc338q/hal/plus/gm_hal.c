...
gm_venc_h264_cnf _gm_venc_h264_cfg[GM_VENC_CHN_NUM] = {0};
gm_venc_mjpg_cnf _gm_venc_mjpg_cfg[GM_VENC_CHN_NUM] = {0};
...
int gm_video_create(char index, hal_vidconfig *config)
{
    ...
    switch (config->codec) {
        ...
        case HAL_VIDCODEC_MJPG: {
            ...
            _gm_venc_mjpg_cfg[index] = mjpgchn;
            break;
            ...
            _gm_venc_h264_cfg[index] = h264chn;
            break;
...
int gm_video_destroy(char index)
{
    ...
    memset(&_gm_venc_h264_cfg[index], 0, sizeof(_gm_venc_h264_cfg[index]));
    memset(&_gm_venc_mjpg_cfg[index], 0, sizeof(_gm_venc_mjpg_cfg[index]));
...
int gm_video_set_bitrate(char index, unsigned int bitrate, unsigned int max_bitrate)
{
    if (!bitrate)
        return -EINVAL;
    if (gm_state[index].payload == HAL_VIDCODEC_UNSPEC)
        return -ENODEV;

    unsigned int bounded_max = max_bitrate < bitrate ? bitrate : max_bitrate;

    switch (gm_state[index].payload) {
        case HAL_VIDCODEC_H264: {
            gm_venc_h264_cnf config = _gm_venc_h264_cfg[index];
            if (!config.dest.width || !_gm_venc_dev[index])
                return -ENODEV;

            switch (config.rate.mode) {
                case GM_VENC_RATEMODE_CBR:
                case GM_VENC_RATEMODE_ECBR:
                    if (config.rate.bitrate == (int)bitrate &&
                        config.rate.maxBitrate == (int)bounded_max)
                        return EXIT_SUCCESS;
                    config.rate.bitrate = bitrate;
                    config.rate.maxBitrate = bounded_max;
                    break;
                case GM_VENC_RATEMODE_VBR:
                case GM_VENC_RATEMODE_EVBR:
                    if (config.rate.bitrate == (int)bitrate &&
                        config.rate.maxBitrate == (int)bounded_max)
                        return EXIT_SUCCESS;
                    config.rate.bitrate = bitrate;
            ...
            int ret = gm_lib.fnSetDeviceConfig(_gm_venc_dev[index], &config);
            if (!ret)
                _gm_venc_h264_cfg[index] = config;
            return ret;
        }
        case HAL_VIDCODEC_MJPG:
        case HAL_VIDCODEC_JPG: {
            gm_venc_mjpg_cnf config = _gm_venc_mjpg_cfg[index];
            if (!config.dest.width || !_gm_venc_dev[index])
                return -ENODEV;
            ...
            switch (config.mode) {
                case GM_VENC_RATEMODE_CBR:
                case GM_VENC_RATEMODE_ECBR:
                    if (config.bitrate == (int)bitrate &&
                        config.maxBitrate == (int)bounded_max)
                        return EXIT_SUCCESS;
                    config.bitrate = bitrate;
                    config.maxBitrate = bounded_max;
                    break;
                case GM_VENC_RATEMODE_VBR:
                case GM_VENC_RATEMODE_EVBR:
                    if (config.bitrate == (int)bitrate &&
            ...
            int ret = gm_lib.fnSetDeviceConfig(_gm_venc_dev[index], &config);
            if (!ret)
                _gm_venc_mjpg_cfg[index] = config;
            return ret;
        }
        default:
            return -EINVAL;
