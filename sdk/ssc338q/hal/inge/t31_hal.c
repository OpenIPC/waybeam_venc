...
int t31_video_set_bitrate(char index, unsigned int bitrate, unsigned int max_bitrate)
{
    if (!bitrate)
        return -EINVAL;
    if (t31_state[index].payload == HAL_VIDCODEC_UNSPEC)
        return -ENODEV;
    if (!t31_venc.fnGetChannelConfig || !t31_venc.fnSetChannelConfig)
        return -ENOTSUP;

    t31_venc_chn channel;
    memset(&channel, 0, sizeof(channel));
    int ret = t31_venc.fnGetChannelConfig(index, &channel);
    if (ret)
        return ret;
    ...
    switch (channel.rate.mode) {
        case T31_VENC_RATEMODE_CBR:
            if (channel.rate.cbr.tgtBitrate == bitrate)
                return EXIT_SUCCESS;
            channel.rate.cbr.tgtBitrate = bitrate;
            break;
        case T31_VENC_RATEMODE_VBR:
            if (channel.rate.vbr.tgtBitrate == bitrate &&
                channel.rate.vbr.maxBitrate == bounded_max)
                return EXIT_SUCCESS;
            channel.rate.vbr.tgtBitrate = bitrate;
            channel.rate.vbr.maxBitrate = bounded_max;
            break;
        case T31_VENC_RATEMODE_AVBR:
            if (channel.rate.avbr.tgtBitrate == bitrate &&
                channel.rate.avbr.maxBitrate == bounded_max)
                return EXIT_SUCCESS;
            channel.rate.avbr.tgtBitrate = bitrate;
            channel.rate.avbr.maxBitrate = bounded_max;
            break;
        case T31_VENC_RATEMODE_CVBR:
            if (channel.rate.cvbr.tgtBitrate == bitrate &&
                channel.rate.cvbr.maxBitrate == bounded_max)
                return EXIT_SUCCESS;
            channel.rate.cvbr.tgtBitrate = bitrate;
            channel.rate.cvbr.maxBitrate = bounded_max;
            break;
        default:
            return -ENOTSUP;
    ...
    return t31_venc.fnSetChannelConfig(index, &channel);
}
