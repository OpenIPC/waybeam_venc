...
typedef struct {
    ...
    int (*fnGetChannelConfig)(int channel, t31_venc_chn *config);
    int (*fnSetChannelConfig)(int channel, t31_venc_chn *config);
...
static int t31_venc_load(t31_venc_impl *venc_lib) {
    ...
    if (!(venc_lib->fnGetChannelConfig = (int(*)(int channel, t31_venc_chn *config))
        hal_symbol_load("t31_venc", venc_lib->handle, "IMP_Encoder_GetChnAttr")))
        return EXIT_FAILURE;

    if (!(venc_lib->fnSetChannelConfig = (int(*)(int channel, t31_venc_chn *config))
        hal_symbol_load("t31_venc", venc_lib->handle, "IMP_Encoder_SetChnAttr")))
        return EXIT_FAILURE;
