...
enum ConfigError parse_list(
    ...
    *count = 0;
    ...
    cursor = ini->str + start_pos;
    limit = (end_pos > 0) ? ini->str + end_pos : ini->str + strlen(ini->str);
    ...
    while (cursor < limit) {
        ...
        if ((size_t)(line_end - line) >= name_len + 1 &&
            strncasecmp(line, param_name, name_len) == 0 &&
            line[name_len] == ':') {
            key_indent = indent;
    ...
    while (cursor < limit && *count < max_entries) {
        const char *line_end = memchr(cursor, '\n', limit - cursor);
        const char *line = cursor;
        int indent = 0;
        ...
        size_t copy_len = line_end - line;
        if (copy_len >= 256)
            copy_len = 255;

        strncpy(entries[*count], line, copy_len);
        entries[*count][copy_len] = '\0';

        char *end = entries[*count] + strlen(entries[*count]);
        while (end > entries[*count] && (unsigned char)end[-1] <= ' ')
            *--end = '\0';
