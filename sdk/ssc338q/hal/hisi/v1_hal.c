...
int v1_video_set_bitrate(char index, unsigned int bitrate, unsigned int max_bitrate)
{
    if (!bitrate)
        return -EINVAL;
    ...
    v1_venc_chn channel;
    memset(&channel, 0, sizeof(channel));
    int ret = v1_venc.fnGetChannelConfig(index, &channel);
    if (ret)
        return ret;
    ...
    switch (channel.attrib.codec) {
        case V1_VENC_CODEC_H264:
            switch (channel.rate.mode) {
                case V1_VENC_RATEMODE_H264CBRv2:
                    channel.rate.h264Cbr.bitrate = bitrate;
                    break;
                case V1_VENC_RATEMODE_H264VBRv2:
            ...
            break;
        case V1_VENC_CODEC_MJPG:
            switch (channel.rate.mode) {
                case V1_VENC_RATEMODE_MJPGCBR:
                    channel.rate.mjpgCbr.bitrate = bitrate;
                    break;
                case V1_VENC_RATEMODE_MJPGVBR:
    ...
    return v1_venc.fnSetChannelConfig(index, &channel);
}
