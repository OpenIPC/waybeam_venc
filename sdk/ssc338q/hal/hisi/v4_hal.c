...
int v4_video_set_bitrate(char index, unsigned int bitrate, unsigned int max_bitrate)
{
    if (!bitrate)
        return -EINVAL;
    ...
    v4_venc_chn channel;
    memset(&channel, 0, sizeof(channel));
    int ret = v4_venc.fnGetChannelConfig(index, &channel);
    if (ret)
        return ret;
    ...
    switch (channel.attrib.codec) {
        case V4_VENC_CODEC_H264:
            switch (channel.rate.mode) {
                case V4_VENC_RATEMODE_H264CBR:
                    channel.rate.h264Cbr.maxBitrate = bitrate;
                    break;
                case V4_VENC_RATEMODE_H264VBR:
                    channel.rate.h264Vbr.maxBitrate = bounded_max;
                    break;
                case V4_VENC_RATEMODE_H264AVBR:
            ...
            break;
        case V4_VENC_CODEC_H265:
            switch (channel.rate.mode) {
                case V4_VENC_RATEMODE_H265CBR:
                    channel.rate.h265Cbr.maxBitrate = bitrate;
                    break;
                case V4_VENC_RATEMODE_H265VBR:
                    channel.rate.h265Vbr.maxBitrate = bounded_max;
                    break;
                case V4_VENC_RATEMODE_H265AVBR:
            ...
            break;
        case V4_VENC_CODEC_MJPG:
            switch (channel.rate.mode) {
                case V4_VENC_RATEMODE_MJPGCBR:
                    channel.rate.mjpgCbr.maxBitrate = bitrate;
                    break;
                case V4_VENC_RATEMODE_MJPGVBR:
    ...
    return v4_venc.fnSetChannelConfig(index, &channel);
}
